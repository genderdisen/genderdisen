    ...
    def collate(self, batch: Dict[str, Any]):
        if self._mode == 'train':
            if self._task == 'ranking':
                ...
                input_ids_query = torch.tensor([item['input_ids_query'] for item in batch])
                segment_ids_query = torch.tensor([item['segment_ids_query'] for item in batch])
                input_mask_query = torch.tensor([item['input_mask_query'] for item in batch])
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        if self._mode == 'train':
            if self._task == 'ranking':
                ...
                input_ids_pos, input_mask_pos, segment_ids_pos = self.pack_bert_features(query_destination_tokens, doc_tokens_pos)
                input_ids_neg, input_mask_neg, segment_ids_neg = self.pack_bert_features(query_destination_tokens, doc_tokens_neg)
                input_ids_query, input_mask_query, segment_ids_query = \
                    self.pack_bert_features(query_destination_tokens, query_tokens)
