    ...
    def __init__(
        ...
        if isinstance(self._dataset, str):
                ...
                for i, line in enumerate(f):
                    ...
                    if self._mode != 'train' or self._dataset.split('.')[-1] == 'json' or self._dataset.split('.')[
                        ...
                        if self._task == 'ranking':
                            query, doc_pos, doc_neg, query_destination = line.strip('\n').split('\t')
            ...
            with open(self._dataset['queries'], 'r') as f:
                for line in f:
                    if self._dataset['queries'].split('.')[-1] == 'json' \
                            or self._dataset['queries'].split('.')[-1] == 'jsonl':
                        line = json.loads(line)
                    else:
            ...
            with open(self._dataset['docs'], 'r') as f:
                for line in f:
                    if self._dataset['docs'].split('.')[-1] == 'json' or \
                            self._dataset['docs'].split('.')[-1] == 'jsonl':
                        line = json.loads(line)
                    else:
    ...
    def collate(self, batch: Dict[str, Any]):
        if self._mode == 'train':
            if self._task == 'ranking':
                ...
                input_ids_q_destination = torch.tensor([item['input_ids_q_destination'] for item in batch])
                segment_ids_q_destination = torch.tensor([item['segment_ids_q_destination'] for item in batch])
                input_mask_q_destination = torch.tensor([item['input_mask_q_destination'] for item in batch])
                ...
                return {'input_ids_pos': input_ids_pos, 'segment_ids_pos': segment_ids_pos,
                        ...
                        'input_ids_q_destination': input_ids_q_destination,
                        'segment_ids_q_destination': segment_ids_q_destination,
                        'input_mask_q_destination': input_mask_q_destination,
                        }
            elif self._task == 'classification':
        ...
        if self._mode == 'train':
            if self._task == 'ranking':
                ...
                doc_tokens_neg = self._tokenizer.tokenize(example['doc_neg'])[
                                 :self._seq_max_len - len(query_tokens) - 3]
                query_destination_tokens = self._tokenizer.tokenize(example['query_destination'])[:self._query_max_len]
                ...
                input_ids_q_destination, input_mask_q_destination, segment_ids_q_destination = \
                    self.pack_bert_features(query_tokens, query_destination_tokens)
