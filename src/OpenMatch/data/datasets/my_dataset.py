...
class Dataset(Dataset):
    ...
    def collate(self, batch: Dict[str, Any]):
        if self._mode == 'train':
            if self._task == 'ranking':
                query_idx = Variable([item['query_idx'] for item in batch])
                query_mask = Variable([item['query_mask'] for item in batch])
                doc_pos_idx = Variable([item['doc_pos_idx'] for item in batch])
                doc_pos_mask = Variable([item['doc_pos_mask'] for item in batch])
                doc_neg_idx = Variable([item['doc_neg_idx'] for item in batch])
                doc_neg_mask = Variable([item['doc_neg_mask'] for item in batch])
                ...
                query_idx = Variable([item['query_idx'] for item in batch])
                query_mask = Variable([item['query_mask'] for item in batch])
                doc_idx = Variable([item['doc_idx'] for item in batch])
                doc_mask = Variable([item['doc_mask'] for item in batch])
                label = Variable([item['label'] for item in batch])
                return {'query_idx': query_idx, 'query_mask': query_mask,
            ...
            retrieval_score = Variable([item['retrieval_score'] for item in batch])
            query_idx = Variable([item['query_idx'] for item in batch])
            query_mask = Variable([item['query_mask'] for item in batch])
            doc_idx = Variable([item['doc_idx'] for item in batch])
            doc_mask = Variable([item['doc_mask'] for item in batch])
