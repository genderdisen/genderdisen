...
class RobertaDataset(Dataset):
                ...
                for i, line in enumerate(f):
                    ...
                    if self._mode != 'train' or self._dataset.split('.')[-1] == 'json' or self._dataset.split('.')[
                        ...
                        if self._task == 'ranking':
                            query, doc_pos, doc_neg, bias_neg, bias_pos = line.strip('\n').split('\t')
    ...
    def pack_roberta_features(self, query_tokens: List[str], doc_tokens: List[str]):
        input_tokens = [self._tokenizer.cls_token] + query_tokens + [self._tokenizer.sep_token] * 2 + doc_tokens + [
        ...
        if self._mode == 'train':
            if self._task == 'ranking':
                ...
                doc_tokens_pos = self._tokenizer.tokenize(example['doc_pos'])[
                                 :self._seq_max_len - len(query_tokens) - 4]
