                ...
                for i, line in enumerate(f):
                    ...
                    if self._mode != 'train' or self._dataset.split('.')[-1] == 'json' or self._dataset.split('.')[
                        ...
                        if self._task == 'ranking':
                            query, doc_pos, doc_neg, bias_neg, attribute_pos, attribute_neg = line.strip('\n').split('\t')
                            line = {'query': query, 'doc_pos': doc_pos, 'doc_neg': doc_neg,
                                    'bias_neg': bias_neg, "attribute_pos": attribute_pos, "attribute_neg": attribute_neg}
            ...
            with open(self._dataset['trec'], 'r') as f:
                ...
                for i, line in enumerate(f):
                    ...
                    if self._mode == 'train':
                        ...
                        self._examples.append(
                            {'query_id': line[0], 'doc_id': line[2], 'attribute_label': int(line[6])})
                    else:
    ...
    def collate(self, batch: Dict[str, Any]):
        if self._mode == 'train':
            if self._task == 'ranking':
                ...
                attribute_pos = torch.tensor([float(item['attribute_pos']) for item in batch])
                attribute_neg = torch.tensor([float(item['attribute_neg']) for item in batch])
            ...
            query_id = [item['query_id'] for item in batch]
            doc_id = [item['doc_id'] for item in batch]
            attribute_label = torch.tensor([item['attribute_label'] for item in batch])
            input_ids = torch.tensor([item['input_ids'] for item in batch])
            segment_ids = torch.tensor([item['segment_ids'] for item in batch])
            input_mask = torch.tensor([item['input_mask'] for item in batch])
            return {'query_id': query_id, 'doc_id': doc_id, 'attribute_label': attribute_label,
                    'input_ids': input_ids, 'segment_ids': segment_ids, 'input_mask': input_mask}
        else:
            raise ValueError('Mode must be `train`, `dev` or `test`.')
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        if self._mode == 'train':
            if self._task == 'ranking':
                ...
                attribute_pos = example['attribute_pos']
                attribute_neg = example['attribute_neg']
                return {'input_ids_pos': input_ids_pos, 'segment_ids_pos': segment_ids_pos,
                        'input_mask_pos': input_mask_pos,
                        'input_ids_neg': input_ids_neg, 'segment_ids_neg': segment_ids_neg,
                        'input_mask_neg': input_mask_neg,
            ...
            return {'query_id': example['query_id'], 'doc_id': example['doc_id'],
                    'attribute_label': example['attribute_label'],
                    'input_ids': input_ids, 'input_mask': input_mask, 'segment_ids': segment_ids}
        else:
            raise ValueError('Mode must be `train`, `dev` or `test`.')
